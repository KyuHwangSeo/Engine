	...
	:IRenderer(device, context)
{
	...
	m_DeferredRTV.resize(5);
	m_DeferredSRV.resize(5);
...
{
	// RenderTarget 설정..
	m_DeviceContext->OMSetRenderTargets(5, &m_DeferredRTV[0], dsv);
...
void Deferred::OnResize(int width, int height)
{
	...
	m_ShadowRT->CreateRTV(renderTargetTextureArray[3].Get(), &rtvDesc);
	m_NormalDepthRT->CreateRTV(renderTargetTextureArray[4].Get(), &rtvDesc);
	...
	m_DeferredRTV[3] = m_ShadowRT->GetRTV();
	m_DeferredRTV[4] = m_NormalDepthRT->GetRTV();
	...
	m_ShadowRT->CreateSRV(renderTargetTextureArray[3].Get(), &srvDesc);
	m_NormalDepthRT->CreateSRV(renderTargetTextureArray[4].Get(), &srvDesc);

	// RenderTarget List 설정..
	m_DeferredSRV[0] = m_AlbedoRT->GetSRV();
	m_DeferredSRV[1] = m_NormalRT->GetSRV();
	m_DeferredSRV[2] = m_PositionRT->GetSRV();
	m_DeferredSRV[3] = m_ShadowRT->GetSRV();
	m_DeferredSRV[4] = m_NormalDepthRT->GetSRV();
